...
public class WeeklyProductPagetestingReadoutUrl {
	public static void main(String args[]) throws IOException {
		File file = new File("C:\\Users\\dines\\OneDrive\\Documents\\ReadoutSaasworthyUrl.txt");
		Scanner scan;
		...
		while (scan.hasNextLine()) {
			...
			try {
				...
				sql = "INSERT INTO weeklyproductpagetesting (productsite,Producttitlename,productName,logo,"
						...
						+ "Articles,Downloads,Customers,Integrations,Slides,Disclaimerlink,Pricinglink,"
						+ "Disclaimerlinktext,Pricinglinktext,faqfeatureslink,faqfeatureslintext,tagline)"
						+ " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
				...
				try {
					boolean Disclaimerlink = docm.select("#pricing > div.disclaimer > a").hasAttr("href");
  

					statement.setBoolean(28, Disclaimerlink);
				} catch (Exception e) {
					statement.setBoolean(28, false);
				}
				try {
					boolean Pricinglink = docm.select("#pricing > center > div > a").hasAttr("href");
 
					statement.setBoolean(29, Pricinglink);
				} catch (Exception e) {
					statement.setBoolean(29, false);
				}
				try {
					Elements link = docm.select("#pricing > div.disclaimer").select("a");
					String Disclaimerlinktext = link.attr("href");
  

					statement.setString(30, Disclaimerlinktext);
				} catch (Exception e) {
					statement.setString(30, null);
				}
				try {
					String Pricinglinktext = docm.select("#pricing > center > div > a").attr("href");
 
					statement.setString(31, Pricinglinktext);
				} catch (Exception e) {
					statement.setString(31, null);
				}
				
				try {
					Boolean faqfeatureslink = docm.select("#faq > div.faq-section > div:nth-child(2) > div > div > p:nth-child(6) > a").hasAttr("href");

					String faqfeatureslintext = docm.select("#faq > div.faq-section > div:nth-child(2) > div > div > p:nth-child(6) > a").attr("href");
					statement.setBoolean(32, faqfeatureslink);

					statement.setString(33, faqfeatureslintext);
				} catch (Exception e) {
					statement.setBoolean(32, false);
					statement.setString(33, null);

				}
				try {
					Boolean tagline = docm.select("body > div:nth-child(12) > div > div.right-overvw > div.head-desc > span").hasText();
 
					statement.setBoolean(34, tagline);
				} catch (Exception e) {
					statement.setBoolean(34, false);
				}
